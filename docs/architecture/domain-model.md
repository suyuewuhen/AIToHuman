...
> **实现现状标注**（对齐 2026-09-10 工作区代码，基线提交 `f196850`）
>
...
> 实现现状（⚠️）：`OrderEvidence` 实体已实现，包含订单、上传者、展示用文件名、MIME、系统生成的存储键、字节数、SHA-256 摘要、创建时间与扫描状态（`Pending`/`Clean`/`Rejected`，终态）。类型白名单（JPEG/PNG/WebP/PDF）与文件签名双重校验，大小上限 5 MB，每单最多 10 份；只有订单服务者能上传，只有订单双方能在扫描通过后通过鉴权下载。
>
> 文件内容走可插拔的 `IFileStorage`：Development 是本机私有目录，生产应替换为私有 Bucket + 短时签名 URL。`IEvidenceScanner` 目前是显式放行的占位实现（会打警告日志），真实病毒/内容扫描、图片重新编码与 EXIF 去除、上传限速、凭证与验收项关联、以及访问审计都还未实现。
...
> - 没有撤回接口。
> - 选择报名的并发安全仅依赖“任务必须为 `Published`”的状态校验，没有数据库事务和并发令牌；并发选单的“最多一份成功”尚未在数据库层得到保证。
