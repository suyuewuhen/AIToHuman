...
| `RiskDecisionEntry` | ✅ | 风险判定的**只追加**留痕：`risk_decision_entries`（迁移 29）一次判定一行，记原因（`RiskDecisionReason`）、结论、命中的规则代码/类别/版本、当时的悬赏金额与时刻；**不可变**（没有修改方法），任务行只保留最新判定、历史在这里，统计口径由它推出，见第 10 节 |
...
- ✅ 每一次风险判定都留痕，且留痕只追加。应用层的每个判定点（创建草稿、编辑草稿、回滚草稿、发布前、已发布任务加价、发布后复检）都会在同一个工作单元里往 `risk_decision_entries` 追加一行 `RiskDecisionEntry`（记 `Reason`/`Verdict`/`RuleCode`/`Category`/`RuleVersion`/`RewardAmount`/`OccurredAt`，没有修改与删除方法）；任务行上的风险字段只表示**最新一次**判定，历史结论一律看留痕表——否则"这条规则拦了多少、误伤多少"在库里无从回答。
...
> 实现现状（⚠️）：领域层已实现 `RiskVerdict`（`Allowed`/`NeedsReview`/`Blocked`）、`RiskReviewStatus`（`NotRequired`/`Pending`/`Approved`/`Rejected`）、`RiskRule`、`RiskAssessment`、`RiskRuleCatalog`（实例目录，内置版本 `BuiltInVersion = 1`）、`RiskRuleCatalogRevision` 与落库形态 `RiskRuleCatalogSnapshot`/`RiskRuleSnapshot`，以及发布后复检的 `RiskEnforcementStatus`（`None`/`RecheckRequired`/`Suspended`）与 `RiskEnforcementOutcome`（`Unchanged`/`FlaggedForRecheck`/`Unpublished`/`OrderFrozen`）、申诉节流与留档的 `RiskAppealPolicy`/`RiskAppealRecord`、判定留痕的 `RiskDecisionEntry`/`RiskDecisionReason`。
>
> - **判定留痕（`RiskDecisionEntry`，迁移 29 `risk_decision_entries`）**：一行就是“某条任务在某次动作后被判成什么”，**只追加、不可变**（没有修改与删除方法）。字段是 `TaskId`、`Reason`（`RiskDecisionReason`：`Created` 创建草稿 / `Edited` 编辑草稿 / `Restored` 回滚草稿 / `Published` 发布前 / `RewardRaised` 已发布任务加价 / `Rechecked` 发布后复检）、`Verdict`、`RuleCode`（放行时为空）、`Category`、`RuleVersion`、`RewardAmount`（阈值规则的依据）与 `OccurredAt`；写入点覆盖**所有会跑判定的动作**（应用层在同一个工作单元里与状态变更一起写），少记一个点统计就会出现“看不见的那部分”。
> - **它和任务上那些风险字段的关系**：`TaskItem` 上的 `RiskVerdict`/`RiskRuleCode`/`RiskCategory`/`RiskSummary`/`RiskRuleVersion`/`RiskAssessedAt` 只表示**最新一次**判定（“这条任务现在是什么状态”），留痕表才是“判定过的每一次”（“这条任务曾经被判成什么、各自是什么原因”）；因此同一条任务会有多行留痕（创建、编辑、发布……各一行），而任务行始终只有一份当前结论。留痕**不记录操作人**——判定是系统按规则跑的，触发它的是所有者做了哪个动作（就是 `Reason`），谁做的动作在各业务表上（例如任务的 `OwnerId`）。
> - **统计口径由留痕推出**（`RiskDecisionService.Summarize`）：窗口内按「规则代码 + 结论」聚合命中次数，叠加**同一窗口内被认定为误伤（`RiskAppealRecord.Status = Accepted`）**的次数，得出“拦了多少 / 其中多少被判成误伤”；窗口默认 30 天、超出 1–365 会被夹到区间内。按任务看轨迹则是同表的 `(TaskId, OccurredAt)` 查询。
>
...
> - 缺失：模型辅助分类与语义判断（现在只有字面词表匹配，改写过的表述可能漏过）；规则目录编辑的双人复核/审批流、灰度或 A/B 与按规则维度的报表；**漏拦**没有度量（命中统计只回答“拦了多少”，没有人工标注的漏拦样本集）；判定留痕里**没有操作人字段**（`RiskDecisionEntry` 只记“因为什么动作判的”即 `Reason`，要追“谁触发的”得回到任务/订单等业务行或 `admin_audit_entries`）；禁止类别命中的已分配任务仍然只有“冻结订单 + 运营按争议处置”，**赔付与退款已能由运营在处置时按金额执行**（见第 8 节），但没有**自动**退款或赔付规则；通知仍只有站内渠道。
