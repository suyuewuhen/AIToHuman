...
- ⚠️ 文件上传、扫描、凭证关联和验收：验收与驳回已实现（驳回必须填原因），凭证文件上传与鉴权下载已实现（类型白名单 + 文件签名校验 + 大小与摘要校验 + 扫描状态机 + 待扫描凭证的退避重扫），存储位置、扫描方式与上传上限都可在运营后台切换/收紧；真实对象存储实现、真实病毒/内容扫描服务、上传限速与“凭证关联到具体验收项”未实现。
