...
运营侧的争议处置：`GET /api/v1/admin/orders?status=&limit=`（管理员身份）返回 `AdminOrderListResponse`——省略 `status` 时默认**只返回 `Disputed`**，传 `all` 看全部；条目另含双方邮箱、提交说明、审批/驳回说明、返工次数与取消信息。`POST /api/v1/admin/orders/{id}/resolve`，body `{ decision, note, amount? }`（`amount` 可选，含义见下文「运营侧的资金视图与处置金额」），`decision ∈ { Approve, Rework, Cancel }`，`note` 必填、≤500 字，返回 `AdminOrderItemResponse`：`Approve` 把订单置为 `Approved`（写审批时间与说明、清空驳回原因）并把任务从 `Assigned` 推进到 `Closed`；`Rework` 退回 `InProgress`、累加返工次数、把处置依据记进驳回原因、任务保持 `Assigned`；`Cancel` 终止订单（`cancelledAt` 落库，`cancelledBy` 为空表示平台处置而不是某个参与者取消，处置依据写进取消原因），任务按“取消订单”的规则回到大厅或直接过期。三种处置都在同一事务里写运营审计（`order.dispute.approve` / `order.dispute.rework` / `order.dispute.cancel`，`targetType=order`，依据写在 `reason`）并通知双方（`order.disputeResolved`，事件键按接收者派生）。

已实现的资金托管与资金流水（**托管与订单状态一起动，失败就不改状态**）：选人成功时在同一个事务里冻结需求方资金（托管额 = 订单悬赏，冻结失败则任务、订单与流水一起回滚）、需求方验收通过全额放款给服务者、取消订单全额退回需求方、争议处置按金额分账。`OrderResponse` 新增 `escrowStatus`（`None`/`Held`/`Released`/`Refunded`/`Settled`）、`escrowAmount`、`releasedAmount`、`refundedAmount`、`paymentReference`、`escrowHeldAt`、`escrowSettledAt`。流水走只读端点 `GET /api/v1/orders/{id}/ledger`：**仅订单参与者**，非参与者返回 `403`（「只有订单参与者可以查看订单。」），订单不存在返回 `404`；响应是 `LedgerEntryResponse[]`（**裸数组，不套 `items`**），每条含 `id`/`orderId`/`taskId`/`kind`（`Hold`/`Release`/`Refund`/`PartialRelease`/`PartialRefund`）/`debitAccount`/`creditAccount`（取值 `OwnerFunds`/`Escrow`/`WorkerPayout`）/`amount`（**恒为正数，转出与转入方向由借/贷账户表达**）/`currency`/`note`/`occurredAt`。因此客户端渲染时不看金额正负，而看 `debitAccount` → `creditAccount`：`OwnerFunds → Escrow` 是下单冻结、`Escrow → WorkerPayout` 是放款（含部分放款）、`Escrow → OwnerFunds` 是退款（含部分退款）。

运营侧的资金视图与处置金额：`GET /api/v1/admin/orders/{id}/ledger`（管理员身份，匿名 `401`、非管理员 `403`）返回同一结构的流水数组，用来排查“这笔单子的钱走到哪了”；`AdminOrderItemResponse` 新增 `escrowStatus`/`escrowAmount`/`releasedAmount`/`refundedAmount`，所以运营的争议队列里就能直接看到托管状态与金额。`AdminResolveDisputeRequest` 新增可选字段 `amount`，**它的含义随 `decision` 而变**：`Approve`（强制完成）时是**放款给服务者的金额**（差额退回需求方），`Cancel`（终止订单）时是**退回需求方的金额**（差额作为补偿给服务者），留空即全额；`Rework`（退回返工）**不涉及资金**，填了 `amount` 返回 `422`「退回返工不涉及资金：赔付或退款金额只在“强制完成”与“终止订单”时填写。」。金额越界同样是 `422`，文案为「赔付金额必须在 0 到托管金额（X）之间。」（X 是当前托管金额，例如「赔付金额必须在 0 到托管金额（100）之间。」）。分账会按金额分别写一条部分放款（`Escrow → WorkerPayout`）与一条部分退款（`Escrow → OwnerFunds`）流水，运营审计的原因里会带上「（资金处置金额 X CNY）」。**托管整体关闭**（运营配置 `payment.provider=disabled`）时，订单的托管字段恒为 `None`/0、任何路径都不写流水，`amount` 也不再产生资金动作——这是本地联调与网关故障时的降级开关。

已实现的风险门禁与人工复核：任务在创建草稿、编辑草稿、回滚草稿与 `POST /tasks/{id}/publish` 前各判定一次确定性风险规则（用的是当下生效的那一版规则目录），结果随任务返回——`TaskResponse` 与 `TaskSummaryResponse` 都带 `riskVerdict`（`Allowed`/`NeedsReview`/`Blocked`）、`riskRuleCode`、`riskCategory`、`riskSummary`、`riskRuleVersion`、`riskReviewStatus`（`NotRequired`/`Pending`/`Approved`/`Rejected`）、`riskReviewNote` 与 `riskPublishBlocked`，`TaskResponse` 另有 `riskAssessedAt` 与 `riskReviewedAt`；**发布后复检的处置**另有三个字段 `riskEnforcementStatus`（`None`/`RecheckRequired`/`Suspended`）、`riskEnforcementReason`、`riskEnforcedAt`，`TaskResponse` 与 `TaskSummaryResponse` 都带，运营复核队列的条目（`AdminRiskReviewItemResponse`）里同一组字段叫 `enforcementStatus`/`enforcementReason`/`enforcedAt`。**`riskPublishBlocked` 是服务端算好的结论，客户端不要自己按 `riskVerdict`/`riskReviewStatus` 推算**（禁止类别、以及需人工复核但尚未放行或已被驳回都为 `true`）；被拦的任务发布返回 `422` 与可读原因（例如“该任务属于平台禁止的类别（prohibited.exam_impersonation · 代考与冒名顶替），不能发布：…”），禁止类别人工也无权放行，草稿仍然保留，用户可以查看原因或自行撤销。运营侧的人工复核接口都要求管理员身份（**匿名 `401`、非管理员 `403`**）：`GET /api/v1/admin/risk/reviews?limit=50` 返回 `AdminRiskReviewListResponse`（条目 `AdminRiskReviewItemResponse`，只列判成 `NeedsReview` 且仍为 `Pending` 的任务，按创建时间升序、先来先处理）；`POST /api/v1/admin/risk/reviews/{taskId}/decide`，body `{ decision: "Approve" | "Reject", note }`，`note` 即依据、必填且 ≤200 字，`Approve` 之后任务可以发布，`Reject` 是终态（规则之后不再命中也不会变回可发布），依据与动作名 `task.risk.approve`/`task.risk.reject` 一起写进 `admin_audit_entries`，任务所有者收到 `task.riskReviewed`（载荷含 `canPublish`）；`GET /api/v1/admin/risk/rules` 返回 `RiskRuleCatalogResponse`（`version`、`highRewardThreshold` 与规则清单，每条给 `code`/`category`/`verdict`/`description`/`keywordCount`，**不返回匹配词本身**）。非法取值返回 `422`（例如“风险复核结论 Maybe 不存在：可选值为 Approve（放行）或 Reject（驳回）。”），对没有待处置复核的任务调用返回 `422`“该任务没有待处置的风险复核。”，重复处置同样是 `422`。
...
已实现的发布后风险复检（规则收紧要管得住**已经在线**的任务）：`POST /api/v1/admin/risk/recheck?limit=`（管理员身份，匿名 `401`、非管理员 `403`）手动跑一轮，返回 `AdminRiskRecheckResponse`——`ruleVersion`（本轮用的规则版本）、`scanned`（本轮候选条数）、`refreshed`（结论没变、只把规则版本号刷新了）、`flagged`（变成需人工复核：任务**保持在线**并回到复核队列）、`unpublished`（命中禁止类别且没有订单，**自动下架**）、`frozen`（命中禁止类别但已有订单，**冻结订单**）、`skipped`（本轮跳过、留给下一轮，例如订单已经结束/已取消/已经在争议里）。`limit` 默认与上限都是 200。单条任务的处置各自写运营审计，手动触发这个请求本身只回传统计、并打一条日志。后台 `RiskRecheckService` 启动 30 秒后跑第一轮、之后每 5 分钟一轮，只扫“仍在线且 `riskRuleVersion` 不是最新”的任务，判完就把版本号刷新，因此不会重复处置；对“同一条规则已经放行过”“已经在队列里等人工”的任务只刷新版本号，**不重新排队、不重复通知**。处置动作的审计是 `task.risk.recheck.unpublish`（自动下架）与 `task.risk.recheck.freezeOrder`（冻结订单），`targetType=task`，操作人是平台自己的固定身份 `00000000-0000-0000-0000-00000000ffff`——运营审计里“人做的”和“系统做的”因此分得开。冻结订单复用争议路径：订单变 `Disputed`、`disputeOpenedBy` 为空（表示不是任何一方发起），运营照旧用 `GET /api/v1/admin/orders?status=` 检索、`POST /api/v1/admin/orders/{id}/resolve` 三种处置。

已实现的追加式判定留痕与规则命中统计：每一次风险判定都会往 `risk_decision_entries` **追加一行**（不更新、不删除），记录这次判定因为什么动作跑的（`reason`：`Created` 创建草稿 / `Edited` 编辑草稿 / `Restored` 回滚草稿 / `Published` 发布前 / `RewardRaised` 已发布任务加价 / `Rechecked` 发布后复检）、结论（`verdict`）、命中的规则代码与类别、`ruleVersion`、当时的悬赏金额与时刻。任务行上只保留**最新一次**判定，历史轨迹在留痕表里。

- `GET /api/v1/admin/risk/stats?days=` 命中统计看板（管理员身份，匿名 `401`、非管理员 `403`）：`days` 是统计窗口天数，省略为 **30**，超出 **1–365** 会被夹到区间内（不是报错）。返回 `RiskDecisionStatsResponse`：`from`/`to`（窗口起止）、`windowDays`（实际生效的天数）、`totalDecisions`（窗口内判定总次数）、`allowedCount`/`needsReviewCount`/`blockedCount`（按结论的分布）、`recheckedCount`（其中由发布后复检触发的次数），以及 `rules[]`——每条是 `RiskRuleHitResponse(code, category, verdict, hits, recheckedHits, firstHitAt, lastHitAt, acceptedAppeals)`：**按「原因代码 + 结论」分组**统计窗口内的命中次数，`acceptedAppeals` 是同一窗口内**被判为误伤（申诉结论 `Accept`）**的次数，因此每条规则都能看到“拦了多少 / 其中多少被判成误伤”。排序是命中次数降序、同次数按代码字典序；`category` 取该分组里第一个非空的类别名（都没有时写 `(未记录类别)`）。
- `GET /api/v1/admin/risk/decisions?taskId=&limit=` 某条任务的判定轨迹（管理员身份，任务不存在时返回空数组而不是 `404`）：`taskId` **必填**——不传时返回空数组，这个接口不是“全量最近判定”的列表；`limit` 省略为 200、会被夹到 1–200（不是报错），按时间升序返回 `RiskDecisionEntryResponse(id, taskId, reason, verdict, ruleCode, category, ruleVersion, rewardAmount, occurredAt)`。留痕里**没有操作人字段**：判定是系统按规则跑的，触发它的是“所有者做了哪个动作”（就是 `reason`）；谁做的这个动作在各业务表上（例如任务的 `ownerId`）。
- 留痕的写入与状态变更在**同一个工作单元**里（应用层的 `IUnitOfWork.Execute` 会开一个事务，留痕与任务/订单的改动一起提交或一起回滚），所以不会出现“任务改了但没留痕”或反之；`risk_decision_entries` 上线之前创建的任务没有历史轨迹，查轨迹会返回空数组（运营页会给出可读提示）。统计接口的取样口径也要知道：窗口内留痕超过 **20 000** 条时只取**最近**的 20 000 条参与聚合（按 `occurredAt` 降序截断），因此 `totalDecisions` 在极端量级下是“样本数”而不是全量。

已实现的申诉节流与留档：申诉队列 `AdminRiskAppealItemResponse` 新增 `appealCount`（这条任务累计申诉过几次，含已经处置过的；表上线之前的老申诉没有留档，显示 `0`）。某条任务的完整申诉轨迹走 `GET /api/v1/admin/risk/appeals/{taskId}/history`（管理员身份，任务不存在 `404`），返回 `AdminRiskAppealHistoryResponse`：`taskId`、`items[]`（每条含 `id`/`ruleCode`/`ruleVersion`/`verdict`/`reason`/`submittedAt`/`status`/`decidedBy`/`decidedByName`/`decidedAt`/`decisionNote`，即“提交时的规则代码与版本、理由、运营结论与依据”），以及当前生效的两条上限 `maxPerTask`（3）与 `maxPerOwnerPerDay`（5）。**超限返回 `422`**，文案分别是「这条任务累计申诉已达上限（3 次）：请先修改文案，规则会重新判定。」与「近 24 小时提交的申诉已达上限（5 次）：请明天再试，或先修改文案。」——次数由服务端在留档表上 COUNT，客户端不要自己按已有条数推算。
...
- `422` 请求语法正确但不符合业务规则；状态机不允许的操作走这里并返回可读中文原因，例如“只有订单参与者可以取消订单。”“服务者只能在开始执行前取消订单，当前状态 InProgress 请与需求方协商后由需求方处理。”“服务者已经提交验收，请先验收或驳回，不要直接取消。”“任务已经分配并产生订单，不能直接撤销：请先处理订单（验收、驳回或取消订单）。”“取消订单必须填写原因，长度不超过 200 个字符。”“该任务的报名已经截止，不能再报名。”“报名截止时间已过，任务不能发布：请撤销后重新创建，或先调整报名截止时间。”“只有订单参与者可以发起争议。”“这条任务累计申诉已达上限（3 次）：请先修改文案，规则会重新判定。”“近 24 小时提交的申诉已达上限（5 次）：请明天再试，或先修改文案。”
- `429` 超过速率限制（写请求按用户或来源 IP 计数，细则见第 12 节）。
...
- **验证方式**：上面这几条幂等契约已经在**主机级端到端用例**里用真 HTTP 打过（真 API 进程 + 临时真库，见 [开发指南](../development/development-guide.md) 第 4 节）：同一个 `Idempotency-Key` 配同一个请求体返回原状态码与原响应体并带 `Idempotency-Replayed: true`，同一个键配不同请求体返回 `409`。同一批用例也用真 HTTP 覆盖了 `/health`、`/health/ready` 的依赖探测（见第 11 节）、写接口限流（见第 12 节）、鉴权与错误映射这几条契约（非管理员访问运营接口 `403`、不存在的订单/任务 `404`、邮箱重复 `409`、缺角色 `422`）。
...
- 已接入的事件类型：`order.created`、`order.statusChanged`、`order.messageCreated`、`order.cancelled`、`order.disputed`、`order.disputeResolved`、`task.expired`、`task.cancelled`、`task.applicationWithdrawn`、`task.riskReviewed`、`task.riskAppealDecided`、`task.riskEnforced`；订单类载荷为 `{ orderId, status, title }`，任务类载荷为 `{ taskId, status, title }`，撤回报名的载荷另含 `{ applicationId, workerId }`。争议发起只通知对方参与者，处置结果通知双方（事件键按接收者派生）。**发布后复检的处置**用 `task.riskEnforced` 通知任务所有者，载荷为 `{ taskId, title, enforcementStatus, verdict, ruleCode, reason }`（只有处置状态、结论、规则代码与原因，不含匹配词）；自动下架时所有者与每个报名中的服务者还各收到一条 `task.cancelled`，冻结订单时双方收到的是复用的 `order.disputed`（事件键前缀 `SuspendedByRisk:`，与参与者自己发起的那条区分开）。复检对“同一条已放行的规则”“已经在队列里等人工”的任务只刷新规则版本号，因此不会重复推送。
...
运营接口挂在 `/api/v1/admin` 下（配置类在 `/admin/settings`，人工兜底类在 `/admin/tasks`、`/admin/orders`、`/admin/users`、`/admin/risk`，订单资金流水在 `/admin/orders/{id}/ledger`，精确地址留痕在 `/admin/address-access`，审计在 `/admin/audits`），都要求管理员身份（部署配置里的 `Admin__UserIds` / `Admin__Emails`，或 `admin` 角色声明）；未配置管理员时一律 `403`。
...
运营配置目录里属于运维的两个分组可以直接改出限流与探针口径：`运维与限流` 下有 `ratelimit.enabled`（写接口限流开关，默认开）、`ratelimit.writesPerMinute`（每分钟写请求上限，默认 240、可配 1–100000）与 `readiness.timeoutSeconds`（就绪检查单项超时，默认 3 秒、可配 1–30）。它们走同一套白名单与审计，改完下一次请求即生效。

## 11. 健康探针

两个探针都在根路径、**不在 `/api/v1` 下**、不需要认证，也不出现在 OpenAPI 的客户端里（它们是给编排系统与运维看的，不是业务 API）：

```text
GET /health          存活（liveness）
GET /health/ready    就绪（readiness）
```

`GET /health` **只看进程是否还在跑**，永远返回 `200`，刻意不碰数据库、Redis 或对象存储——依赖挂了重启进程解决不了问题，只会让所有实例一起抖：

```json
{ "status": "healthy", "service": "AIToHuman.Api", "utc": "2026-09-13T08:00:00+00:00" }
```

`GET /health/ready` **逐项探测依赖**：数据库、Redis 与文件存储并行探测、各自计时，**全部健康才返回 `200`**，只要有一项失败就返回 `503`（响应体结构与成功时一致），编排系统据此把实例从负载均衡摘掉。响应多出 `durationMs` 与 `checks[]`：

```json
{
  "status": "unhealthy",
  "service": "AIToHuman.Api",
  "utc": "2026-09-13T08:00:00+00:00",
  "durationMs": 128,
  "checks": [
    { "name": "postgres", "status": "ok", "detail": "连接正常，已应用的迁移 29 个，没有待应用的迁移。", "durationMs": 42 },
    { "name": "redis", "status": "skipped", "detail": "没有开启多实例通知扇出，本次不探测 Redis。", "durationMs": 0 },
    { "name": "storage", "status": "failed", "detail": "local 存储不可用：目录不可写。", "durationMs": 86 }
  ]
}
```
...
- **`status` 是三态**，不是布尔：`ok`（依赖可用）、`failed`（不可用，整体结论变不健康）、`skipped`（**当前配置下用不上这个依赖，不影响结论**——没开多实例通知扇出时不探测 Redis，否则单实例部署会因为没配 Redis 而永远“未就绪”）。
- **`postgres`**：能否连上，**以及有没有待应用的迁移**。库连得上但少了迁移，接口照样会在运行时炸，因此这项一并读迁移历史；待应用迁移时 `detail` 会写明“已应用 N 个、还有 M 个没应用”。
- **`redis`**：只在 `notifications.fanout.enabled=true` 时参与。开关开着但部署配置里没有连接串时这项直接 `failed`（并提示补连接串或关掉开关）。
- **`storage`**：本机目录实现会**写入再删除**一个探针对象（`readiness/probe.txt`），能抓到“目录不存在/没权限/磁盘满”；对象存储实现对该探测键做一次**存在性检查**（返回“不存在”也算成功，关键是签名与凭据被服务端接受）。业务数据不会被这个探针碰到。
- **单项超时**由运营配置 `readiness.timeoutSeconds` 控制（默认 3 秒、可配 1–30），超时按该项不健康处理；**任何异常都会被折成该项的 `failed` 而不是冒泡成 `500`**——就绪探针自己挂掉是最没用的监控信号。
- 探针**不写业务状态、不做鉴权、可被负载均衡高频调用**，客户端不要把它当成业务健康接口来轮询。

## 12. 写接口限流

写请求按分区计数，超限返回 `429`；读请求与健康探针完全不受影响（限流是为了挡脚本刷写，不是让人打不开页面）。

- **限哪些**：`/api/v1` 下的 `POST`/`PUT`/`PATCH`/`DELETE`。`GET` 等读请求、`/health` 与 `/health/ready`、SignalR 协商都不计数。注意 `POST /api/v1/ai/plan/stream`（SSE 长连接）**照样计入写请求**——它是一次写调用（幂等中间件排除它，不代表限流也排除），因此对话轮次也受同一份分钟额度约束。
- **按谁计数（分区）**：已认证请求按**用户 ID**（换 IP 也绕不过）；未认证请求按**来源 IP**（注册与登录是匿名写请求，否则撞库没有成本）。代价是同一出口 IP 后面的人共用一份额度，反向代理部署时必须由代理转发真实来源。
- **窗口口径**：自然分钟（对齐 UTC 整分钟）的**固定窗口**，不是滑动窗口——好解释、好排查；代价是窗口交界处最多容许两倍瞬时速率，对本场景足够。
- **上限**：运营配置 `ratelimit.writesPerMinute`，默认 **240/分钟**、可配 1–100000；关掉 `ratelimit.enabled` 则完全不做计数（抢修时的一键降级，改完下一个请求生效）。
- **响应头**：每个被计数的请求都会带回 `X-RateLimit-Limit`（当前上限）、`X-RateLimit-Remaining`（本窗口剩余次数）与 `X-RateLimit-Partition`（这份额度算在谁头上：`user:{id}` 或 `ip:{地址}`）——排查限流时最常见的问题是“我跟别人共用一个 IP”，只看剩余次数是看不出来的。被拒时另带 `Retry-After`（还要等多少秒，至少 1）。
- **拒绝响应体**是 `429` + 可读中文原因（`title`/`status`/`detail`/`instance`）：

```json
{
  "title": "请求过于频繁",
  "status": 429,
  "detail": "一分钟内的写请求次数已达上限（240 次），请在 37 秒后重试。",
  "instance": "/api/v1/tasks/01J.../applications"
}
```

- **一条豁免**：`/api/v1/admin/settings` 的写入**永不限流**（路径前缀匹配，含单条 GET/PUT/DELETE 与自检）。上限本身是配置项，如果配小了连改回来的请求都被挡，运营就会把自己锁在门外整整一个窗口；其余运营写接口（处置争议、复检、下架等）照常计数。
- **与幂等键的顺序**：限流在幂等中间件**之前**执行，因此被限流拒绝的 `429` **不会**被写成某个 `Idempotency-Key` 的处理结果。客户端等窗口过去后**可以用同一个键重试**，会真正执行；如果顺序反过来，这个 `429` 会被缓存并在重试时一直回放（见第 7 节）。
- **单实例计数（重要）**：计数器在 API 进程内存里，进程重启即清零，多实例部署时**每个实例各记一份**，实际配额是“实例数 × 上限”。它是防脚本刷写的粗糙闸门，**不是分布式限流**，也不能当作计费或风控配额。
- **验证方式**：这条契约已在**主机级端到端用例**里用真 HTTP 打过——把 `ratelimit.writesPerMinute` 配成 2，第三个写请求得到 `429` + `Retry-After` + `X-RateLimit-*`，同时断言读接口不受影响、配置写入不被限流（见 [开发指南](../development/development-guide.md) 第 4 节）。
