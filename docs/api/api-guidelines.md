...
- **验证方式**：上面这几条幂等契约已经在**主机级端到端用例**里用真 HTTP 打过（真 API 进程 + 临时真库，见 [开发指南](../development/development-guide.md) 第 4 节）：同一个 `Idempotency-Key` 配同一个请求体返回原状态码与原响应体并带 `Idempotency-Replayed: true`，同一个键配不同请求体返回 `409`。同一批用例也用真 HTTP 覆盖了 `/health`、鉴权与错误映射这几条契约（非管理员访问运营接口 `403`、不存在的订单/任务 `404`、邮箱重复 `409`、缺角色 `422`）。
