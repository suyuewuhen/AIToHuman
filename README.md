...
- ✅ 禁止任务会被拦截，高风险任务不会自动进入大厅（规则目录固定在代码里并带版本号，当前 12 个原因代码：代考冒名、违禁品、欺诈与绕过身份核验、跟踪偷拍、危害人身安全、持证医疗行为一律不能发布，人工也无权放行；证件与重要文件、受限场所、敏感物品与照护对象、用途不明的物品，以及悬赏超过 5000 元或截止时间落在北京时间 00:00–06:00 的任务转人工复核，运营放行后才可发布、驳回即不能发布。被拦下的任务可以申诉：转人工被驳回的，运营认定误判即可放行；**禁止类别命中的，申诉成立也不会放行**，只记为规则误伤并提示改文案后重新判定。仍缺模型辅助分类与申诉的次数/频率限制。
